...
import 'dart:convert';
import '../Individual read/InPopular.dart';
...
class ReviewItem {
  ...
  ReviewItem({
  required this.name, required this.image, required this.rating, required this.cont});
}
  ...
  Future<void> fetchPop() async{
    ...
    try{
      ...
      if(response.statusCode == 200){
        ...
        final List<ReviewItem> fetchedBook = jsonData.map((data) => ReviewItem(name: data['name'], image: data['image'], rating: data['rating'], cont: data['cont'], )).toList();
        setState(() {
          print("Data Received");
          items = fetchedBook;
        });
          ...
          return GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => BookDetailsPage(reviewItem: book),
                ),
              );
            },
            child: Container(
